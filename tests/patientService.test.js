...
it('deve fazer rollback se falhar ao criar paciente', async () => {
  ...
  expect(pool.query).toHaveBeenCalledWith('DELETE FROM User WHERE id = ?', [1]);
});
...
it('deve atualizar parcialmente', async () => {
  pool.query
    .mockResolvedValueOnce([
      [{ id: 1, nome: 'old', telefone: 'old', user_id: 1 }],
    ])
...
it('deve erro se paciente não existir', async () => {
  ...
  await expect(PatientService.patchPatientById(1, {})).rejects.toThrow(
...
it('deve erro se não passar id', async () => {
  await expect(PatientService.deletePatient()).rejects.toThrow(
...
it('deve atualizar a senha quando password é enviado no patchPatientById', async () => {
  ...
  expect(resultado.message).toBe(
    'Paciente atualizado parcialmente com sucesso',
  );
...
it('deve lançar erro quando patient_id não for informado no patchPatientById', async () => {
  ...
  await expect(PatientService.patchPatientById(null, data)).rejects.toThrow(
    'ID do paciente é obrigatório',
  );
});
...
it('deve retornar pacientes', async () => {
  Patient.getPatientsByClinic.mockResolvedValue([{ id: 1, nome: 'Lucas' }]);
...
it('deve lançar erro', async () => {
  Patient.getPatientsByClinic.mockRejectedValue(new Error('erro'));

  await expect(PatientService.getPatientsByClinicId(1)).rejects.toThrow('erro');
});
