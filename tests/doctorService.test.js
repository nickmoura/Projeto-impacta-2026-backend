...
describe("DoctorService - createDoctor", () => {
    ...
    it("deve criar um médico com sucesso", async () => {
        ...
        expect(Clinic.getClinicByCNPJ).toHaveBeenCalledWith("123");
    ...
    ).rejects.toThrow("Todos os campos sao obrigatórios");

    });


it("deve lançar erro se a clínica não existir", async () => {
    ...
    ).rejects.toThrow("Clínica não encontrada com o CNPJ fornecido");
...
it("deve deletar user se falhar ao criar médico", async () => {
    ...
    Doctor.createDoctor.mockRejectedValue(
        new Error("erro")
    );
...
describe("getDoctor_by_id", () => {
    ...
    it("deve lançar erro se médico não existir", async () => {
        ...
        await expect(
            DoctorService.getDoctor_by_id(1)
...
describe("deleteDoctor_by_id", () => {
    ...
    it("deve deletar médico", async () => {
        ...
        expect(Doctor.deleteDoctorById)
            .toHaveBeenCalledWith(1);

        expect(User.deleteUserById)
...
it("deve atualizar medico com sucesso", async () => {
    Doctor.getDoctorById.mockResolvedValue({
        id: 1,
        user_id: 10
    });
...
it("deve lançar erro se medico nao existir", async () => {
    Doctor.getDoctorById.mockResolvedValue(null);

    await expect(
        DoctorService.putDoctor_by_id(1, {})
    ).rejects.toThrow("Médico não encontrado");
});

it("deve deletar medico com sucesso", async () => {
    Doctor.getDoctorById.mockResolvedValue({
        id: 1,
        user_id: 10
    });
...
it("deve lançar erro ao deletar medico inexistente", async () => {
    Doctor.getDoctorById.mockResolvedValue(null);

    await expect(
        DoctorService.deleteDoctor_by_id(1)
    ).rejects.toThrow("Médico não encontrado");
});

it("deve atualizar medico com nova senha", async () => {
    Doctor.getDoctorById
        .mockResolvedValueOnce({
            id: 1,
            user_id: 10
